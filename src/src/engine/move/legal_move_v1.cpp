...
namespace pdp_chess {
        ...
        if (is_white) {
            for (uint8_t index : getPositionsV1(board.pieces[white]->pawns.value)) { //white
                if ((index >= 8) & (index <= 15)) { //pawn base position
                    if (!((board.pieces[white]->all.value >> (index + 16)) & 1) &
                        !((board.pieces[white]->all.value >> (index + 8)) & 1) &
                        !((board.pieces[black]->all.value >> (index + 16)) & 1) &
                        !((board.pieces[black]->all.value >> (index + 8)) & 1)) {
                        Move mv = {.start_position = index, .start_type = type, .target_position = (uint8_t) (index +
                                                                                                              16)};
                        res.push_back(mv);
                    }
                }
                if (!((board.pieces[white]->all.value >> (index + 8)) & 1) &
                    !((board.pieces[black]->all.value >> (index - 8)) & 1)) { //advance from one
    ...
    void LegalMoveV1::moveLineVerticalHorizontal(const Board &board, uint8_t position_start, bool color_piece,
                                                 std::vector<Move> &res) {
        uint8_t index2;
    ...
    void
    LegalMoveV1::moveDiagonal(const Board &board, uint8_t position_start, bool color_piece, std::vector<Move> &res) {
        uint8_t index2;
        index2 = position_start;
        for (int i = position_start % 8 + 1; i < 8; i++) { //diagonal up right
            index2 = index2 + 9;
            if (((board.pieces[color_piece]->all.value >> index2) & 1) ||
                (index2 > 63)) { //piece of its color or move leave the board
                break;
            } else if ((board.pieces[!color_piece]->all.value >> index2) & 1) {
                Move mv = {.start_position = position_start, .target_position = index2};
                res.push_back(mv);
        ...
        for (int i = position_start % 8 + 1; i < 8; i++) { //diagonal down right
            index2 = index2 - 7;
            if (((board.pieces[color_piece]->all.value >> index2) & 1) || (index2 < 0) ||
                (index2 > 63)) { //piece of its color or move leave the board
                break;
            } else if ((board.pieces[!color_piece]->all.value >> index2) & 1) {
                Move mv = {.start_position = position_start, .target_position = index2};
    ...
    void LegalMoveV1::moveThisPosition(const Board &board, uint8_t position_start, uint8_t position_target,
                                       uint8_t position_x_target, bool color_piece, std::vector<Move> &res) {
        if ((position_target < 0 | position_target > 63) | (position_x_target < 0 | position_x_target > 7) |
            (board.pieces[color_piece]->all.value >> position_target) & 1) {
            return;
        }
